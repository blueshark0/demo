...
1. 打开一个练习文件，先读顶部块注释里的 **【知识点】**（概念与思路）和 **【本关任务】**（这一关要达成的目标）。
2. 找到文件里所有 `// TODO`，**自己动手把代码写出来**实现每个 TODO 描述的目标。每个 TODO 只告诉你「要做什么」，不会告诉你「怎么写」——这正是练习的意义。
3. 运行 `main` 方法。
   - 如果抛出 `UnsupportedOperationException`，说明还有方法体的 `TODO` 没补全。
   - 如果输出为空，或某个值是 `0` / 空字符串 / `null` 之类占位值，说明对应的 `TODO` 还没写。
4. 把控制台输出与文件底部 **「预期输出」** 逐行对照，**完全一致就过关**！
5. 卡住时：回头重读 **【知识点】**，再对照 **【本关任务】** 的目标和底部「预期输出」反推该写什么。本教程**不提供参考答案**，鼓励多试错、多查阅 JDK 文档。
